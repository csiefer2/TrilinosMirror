...
\cbb{aggregation: distance laplacian directional weights}{Array(double)}{{1,1,1}}{Directional weights for the distance laplacian}
...
\cbb{semicoarsen: number of levels}{int}{0}{Determines the maximum number of times that semicoarsening is applied.}
...
\cbb{sa: nodal damping factor}{double}{1.33}{Damping factor for smoothed aggregation for the nodal smoothing in Maxwell1.}
...
\cbb{sa: eigen-analysis type}{string}{power-method}{Algorithm for eigen-analysis for SA.  Options: "power-method" (default) or "cg".}
...
\cbb{sa: rowsumabs replace single entry row with zero}{bool}{true}{If a row has a single entry, set the inverse of the diagonal entry to be zero.}
        
\cbb{sa: keep tentative prolongator}{bool}{false}{Keep the tentative prolongator instead of dropping it once it has been smoothed.}
        
\cbb{replicate: npdes}{int}{1}{ Used with multigrid algorithm 'replicate' to transform scalar prolongator to one  for PDE systems with npdes dofs per node. }
...
\cbb{filtered matrix: lumping choice}{string}{"diag lumping"}{Can be no lumping (no modification to kept entries), diag lumping (modify diagonal to maintain row sums), or distributed lumping (modify kept nonzeros in a fashion proportional to their magnitude to preserve row sums). }
        
\cbb{filtered matrix: use spread lumping}{bool}{false}{Lump (but not necessarily to the diagonal, trying not to lose diagonal dominance more than the original matrix) dropped entries during the construction of a filtered matrix. This allows user to preserve constant nullspace. Only used when filtered matrix: use lumping is set to true. }
        
\cbb{filtered matrix: spread lumping diag dom growth factor}{double}{1.1}{Determines how much a measure of diagonal dominance is allowed to grow when using spread lumping. Only used when filtered matrix: use lumping and filtered matrix: use spread lumping are both set to true. }
        
\cbb{filtered matrix: spread lumping diag dom cap}{double}{2.0}{For matrix rows that are very diagonally dominant, we ignore the diag dom growth factor and instead use this simple cap as to how large the diagonal dominance measure should allowed to be after dropping. Only used when filtered matrix: use lumping and filtered matrix: use spread lumping are both set to true. }
...
\cbb{filtered matrix: count negative diagonals}{bool}{false}{Counts the number of negative diagonals (involves MPI communication).}
...
\cbb{repartition: use subcommunicators in place}{bool}{false}{Instead of repartitioning, we just removeEmptyProcessesInPlace, using a map to provide the new communicator.}
...
\cbb{repartition: put on single proc}{int}{0}{Number of rows below which we always repartition to a single rank}
        
\cbb{repartition: send type}{string}{""}{Tpetra 'Send type' to use in repartitioning}
...
\cbb{rap: fix zero diagonals replacement}{double}{1.}{Replacement value for diagonal entries that are considered zero.}
